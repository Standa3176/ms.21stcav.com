...
status: planning
stopped_at: Completed quick task 260606-lhp — decision-grade dashboard tiles shipped. Two new StatsOverviewWidgets (HighConfidenceSourceableWidget + SuggestionsQueueHealthWidget) replace the NPO Stat on PendingReviewsWidget; both consume from new SnapshotAggregator::computeSuggestionsTriageHealth() (cached via dashboard_snapshots, refreshed every 5 min). Shared Suggestion::scopeHighConfidenceSourceable extracted so badge + tile + aggregator share one predicate (drift-prevention test pins them together). Focused Pest 4/4; full suite 1,811 / 219 / 3 (+8 pass / −4 fail vs 260606-gnu — zero new failures). Earlier today: 260606-c4o env() guardrail + 260606-gnu inbox triage. Phase 12 still complete; Phase 13 (E3 WhatsApp) still ready for planning.

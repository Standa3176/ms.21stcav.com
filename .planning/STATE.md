...
status: planning
stopped_at: Completed quick task 260607-pys — `/admin/ad-candidates` Filament page shipped for Google Ads campaign planning. New AdCandidateScanner service (single source of truth for the golden-target predicate: live + margin >= £X + stock + beats lowest competitor) — BackfillMerchantFeedCommand refactored to delegate; grep confirms `competitor_prices` SQL no longer in the command. Page has Brand multi-select + Min margin £ (default 199) + Stock-required + Beat-required toggles; default sort by margin DESC; per-row "View on storefront" + "Edit in admin"; bulk Copy SKU CSV for Google Ads upload + Send-to-Google-Ads placeholder (Phase 15). New AdCandidatesReadyWidget tile on home dashboard, cached via SnapshotAggregator::computeAdCandidatesHealth(). Focused Pest 37/37 GREEN; full suite 1,911/222/3 vs 260607-hxa 1,896/219/3 (+15 pass + 3 pre-existing fails caused by 260607-hxa's EanSearch enum bumping IntegrationHealthWidget count to 6 — hardcoded test assertions to fix). Earlier today: 260607-9c6 + 260607-cgd + 260607-g25 + 260607-hxa. Yesterday: 9 quick tasks. Phase 12 still complete; Phase 13 (E3 WhatsApp) still ready for planning.

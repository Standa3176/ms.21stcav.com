...
**Lesson captured (don't repeat):** Never call `env()` outside `config/*.php` files when config:cache is in use. Always bind via `env()` inside a config file, then read via `config('foo.bar')` from app/route code. Three schedule toggles silently fell off-air post-cutover because of this anti-pattern.

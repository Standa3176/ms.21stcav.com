...
status: planning
stopped_at: Completed quick task 260606-gnu — Suggestions inbox triage shipped. `suggestions:prune-orphans` artisan command (Mon 06:00 London cron, BEFORE supplier sync) auto-rejects stale competitor-only orphans (>30d + off-supplier + <2 competitors). Filament nav badge rewritten to show "high-confidence sourceable" count only (was raw pending — 14k noise); new tooltip exposes three-tier breakdown. Focused Pest 5/5; zero new full-suite regressions. Earlier today shipped 260606-c4o env() architectural guardrail. Phase 12 still complete; Phase 13 (E3 WhatsApp) still ready for planning.
last_updated: "2026-06-06T13:00:00.000Z"
...
| 2026-06-06 | **Suggestions inbox triage.** `suggestions:prune-orphans` artisan command auto-rejects stale competitor-only orphans (pending + new_product_opportunity + sku NOT EXISTS in supplier_sku_cache + supporting_competitors<2 + created_at older than `--days=30`). Scheduled Mon 06:00 London (cron `0 5 * * 1` UTC) so it runs *before* `supplier:db-sync` (07:00) and `supplier:refresh-sku-cache` (07:05) — today's-fresh suggestions are never pruned. Driver-aware JSON expression (MySQL `JSON_UNQUOTE`/`UNSIGNED` vs SQLite `json_extract`/`INTEGER`) so the Pest test runs on in-memory SQLite. Filament navigation badge rewrite: was raw pending count (~14k noise); now counts "high-confidence sourceable" only (pending + new_product_opportunity + EXISTS in supplier_sku_cache + supporting_competitors≥3) — actionable signal. New `getNavigationBadgeTooltip()` exposes the three-tier breakdown (`N high-confidence • M sourceable • K raw`), cached 60s. Empirical dev DB: `--dry-run` reported 17,164 candidates. Focused Pest 5/5 (18 assertions, 5.59s); full suite delta vs 260606-c4o baseline = zero new failures (1,803 passed / 223 pre-existing failures / 3 skipped — Suggestion subset stable at 105 passed / 10 pre-existing failed). Quick task [260606-gnu](./quick/260606-gnu-inbox-triage-suggestions-prune-orphans-h/) | feat(suggestions) | ed893b8 + d6c8a4d + 69223cf + d853a27 |

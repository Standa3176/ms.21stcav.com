...
status: planning
stopped_at: Completed quick task 260607-hxa — swapped Icecat for EAN-search.org as the default EAN lookup provider after this morning's 260607-g25 Icecat fallback returned 0/116 matches on premium B2B AV SKUs. New `EanSearchClient` (MPN→GTIN reverse lookup, brand-name match heuristic, €0.003/query, free tier 100/day). New `IntegrationCredentialKind::EanSearch` (token field, admin auto-form, TestIntegrationAction wired). Provider switchable via `config('integrations.ean_fallback_provider')` defaulting to 'ean_search'; flip to 'icecat' via `EAN_FALLBACK_PROVIDER=icecat` for forensic A/B. CLI flags unchanged (`--icecat-fallback` stays for muscle memory, internally provider-agnostic). Outcome buckets renamed `recovered_from_icecat` → `recovered_from_ean_lookup` etc. IcecatClient retained for SourceProductImagesCommand image lookups. Operator must sign up at https://www.ean-search.org and paste token via /admin/integration-credentials post-deploy. Pest +15 GREEN; full suite 1,896/219/3 vs 260607-g25 baseline 1,881/219/3 = 0 new fails. Earlier today: 260607-9c6 HIGH security remediation + 260607-cgd backfill-merchant-feed + 260607-g25 Icecat fallback. Yesterday: 9 quick tasks + security review. Phase 12 still complete; Phase 13 (E3 WhatsApp) still ready for planning.

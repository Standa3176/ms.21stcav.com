...
| 2026-05-25 | Supplier cost accuracy: buy_price = cheapest IN-STOCK supplier (was latest-updated; 1,772 SKUs re-costed live) + `supplier:explain-cost {sku}` diagnostic + `--flag-obsolete` (no-supplier products → pending; 160 demoted live). Pricing Ops dashboard UX: clickable tiles → filterable modal + CSV/XLS export, active-nav "you are here" highlight, + "Products to add" tile (parts on ≥2 suppliers not on MS, via `supplier:scan-add-candidates`, weekly Sun 05:00). 10 tests green. Quick task [260525-szo](./quick/260525-szo-supplier-cost-accuracy-cheapest-in-stock/) | fix(sync)+feat(pricing) | 058fd96 |

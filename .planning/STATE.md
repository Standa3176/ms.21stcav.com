...
status: planning
stopped_at: Completed quick task 260607-g25 — Icecat EAN fallback for backfill-merchant-feed. Today's prod run uncovered 0/116 EAN recovery on premium B2B SKUs (Panasonic PT- projectors, Sony FW-Bravia, PTZOptics, etc) because distributors don't publish GTIN to supplier_db for channel-only products. New `IcecatClient::lookupGtinByMpn(brand, mpn)` extracts GTIN from Icecat's brand+MPN query response (handles 3 schema variants: GeneralInfo.GTIN, GeneralInfo.GTINs[].Value, EANCodes[]). `BackfillMerchantFeedCommand` wired with `--icecat-fallback` (default ON), `--max-icecat-spend-pence=200` (£2 cap, 0.2p/query), 4 new outcome buckets (recovered_from_icecat / icecat_no_match / icecat_invalid_ean / icecat_budget_exhausted), and a 7-row dry-run table replacing the previous 4-row. Backwards-compatible: `--no-icecat-fallback` produces byte-identical 4-row output. Pest +14 cases GREEN (8 IcecatClient unit + 6 BackfillCommand cases A-F); full suite 1881/219/3 vs 260607-cgd baseline 1867/219/3 = 0 new fails. Earlier today: 260607-9c6 HIGH security remediation + 260607-cgd backfill-merchant-feed command. Yesterday: 9 quick tasks shipped + security review. Phase 12 still complete; Phase 13 (E3 WhatsApp) still ready for planning.

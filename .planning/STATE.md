...
status: planning
stopped_at: Completed quick task 260607-cgd — `products:backfill-merchant-feed` ships to fix 89% Google Merchant Center disapproval. Today's diagnostic established 3,922 live products / 3,493 triple-fail (no EAN AND no brand) / 94% EAN recoverability from supplier_db. The new command backfills EAN (free, deterministic, normaliseEan trait validation rejects "N/A"/"0"/empty), brand (free, TaxonomyResolver fuzzy ≥0.85), category (Claude ~£35 for full 3,490 set). `--resync` chain pushes only successfully-updated SKUs to Woo so the Google Listings & Ads plugin re-feeds Merchant Center. Refactored `normaliseEan()` to `App\Console\Concerns\NormalisesEan` trait (drift-prevention; GenerateProductDraftsCommand retrofitted). Focused Pest 27/27 GREEN; full suite +22 pass / 0 new fails vs 260607-9c6. Earlier today: 260607-9c6 HIGH security remediation. Earlier yesterday: 8 quick tasks shipped + security review. Phase 12 still complete; Phase 13 (E3 WhatsApp) still ready for planning.

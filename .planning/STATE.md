...
status: planning
stopped_at: Completed quick task 260607-9c6 — both 2026-06-06 HIGH security findings remediated. H-1 GDPR webhook_receipts prune: new `webhooks:prune-receipts` artisan + Mon-Sun 03:25 London cron with per-topic retention (order=30d, customer=7d, other=90d); slots between integration-events:prune (03:10) and sync-diffs:prune (03:30). H-2 mysqldump credential leak: WooDbSnapshotter refactored to write a chmod-0600 .cnf tempfile (`[client]\nuser=…\npassword=…`) and pass via `--defaults-extra-file=<path>` — password no longer in /proc/*/cmdline argv. Sentinel-password regression test (`SECRET_SHOULD_NOT_LEAK_123!@#`) catches any future revert. SECURITY-REVIEW.md frontmatter flipped to `status: high_findings_remediated`. Focused tests 22/22 GREEN; full suite +12 pass / 0 new fails vs 260606-rld baseline. Earlier today: 260606-c4o env() + 260606-gnu inbox triage + 260606-lhp dashboard tiles + 260606-mx9 auto-create health page + 260606-o63 auto_create_status fix + 260606-p4q bell notifications + 260606-q7h resolver audit + 260606-rld pricing-ops brand + 1c76e63 app-wide security review. Phase 12 still complete; Phase 13 (E3 WhatsApp) still ready for planning.

...
it('resolveBrand returns matching term id', function (): void {
    $woo = fakeWooClientReturning([
        '/products/attributes/pa_brand/terms' => [
            ['id' => 42, 'name' => 'Logitech'],
            ['id' => 43, 'name' => 'Lenovo'],
        ],
    ]);
...
it('uses configured brand_taxonomy slug from config', function (): void {
    config()->set('product_auto_create.brand_taxonomy', 'product_brand');

    $mock = Mockery::mock(WooClient::class);
    $mock->shouldReceive('get')
        ->with('/products/attributes/product_brand/terms', Mockery::any())
        ->once()
        ->andReturn([['id' => 5, 'name' => 'ACME']]);

    $resolver = new TaxonomyResolver($mock);
